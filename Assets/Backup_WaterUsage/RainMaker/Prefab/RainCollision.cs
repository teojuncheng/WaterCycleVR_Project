...
namespace DigitalRuby.RainMaker
{
    public class RainCollision : MonoBehaviour
    {
        ...
        public AudioSource RainAudio; // <-- Add this

        private void Start()
        {
            if (RainAudio != null)
            {
                RainAudio.loop = true; // make sure it loops
            }
        }

        private void Update()
        {
            // Optional: keep audio in sync with particle system
            if (RainParticleSystem != null && RainAudio != null)
            {
                if (RainParticleSystem.isPlaying && !RainAudio.isPlaying)
                {
                    RainAudio.Play();
                }
                else if (!RainParticleSystem.isPlaying && RainAudio.isPlaying)
                {
                    RainAudio.Stop();
                }
            }
